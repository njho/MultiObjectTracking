...
class RecurrentDetectionForwarder(Forwarder):
    def __init__(self, engine, extraction_keys=DETECTION_EXTRACTION_KEYS):
        ...
        self.interactive_visualization = self.config.bool(
    ...
    def _forward_video(self, n_timesteps, tag):
        ...
        for t_start in range(0, n_timesteps, batch_size):
            ...
            for j in range(batch_size):
                ...
                img_filename = extractions[DataKeys.IMAGE_FILENAMES][0][j].decode(
                    "utf-8"
                )
                print(img_filename)
                if self.export_for_kitti_semseg:
                    ...
                    _export_detections_kitti_format(
                        out_folder,
                        det_boxes,
                        det_classes,
                        det_masks,
                ...
                if self.visualize_detections:
                    if DataKeys.RAW_IMAGES not in extractions:
                        print(
                            "Can't extract raw images for visualization, maybe images in batch have different size?",
                            file=log.v5,
                        )
                        assert False
    ...
    def _forward_timestep(self, recurrent_states):
        ...
        step_res = self.trainer.validation_step(
            feed_dict=feed_dict, extraction_keys=self._extraction_keys
        )
        ...
        if Extractions.RECURRENT_STATE in extractions:
            ...
            # flatten
            recurrent_states = [
                item for statetuple in recurrent_states for item in statetuple
            ]
            recurrent_states = [
                x[np.newaxis] if len(x.shape) == 1 else x for x in recurrent_states
            ]
        return recurrent_states, measures, extractions


def _export_detections_kitti_format(
    out_folder, det_boxes, det_classes, det_masks, det_scores, t, tag
):
    ...
    with open(out_folder + "pred_list/" + image_name + ".txt", "w+") as f:
        for idx, (bbox, score, class_, mask) in enumerate(
            zip(det_boxes, det_scores, det_classes, det_masks)
        ):
            mask_img_filename = (
                "pred_img/" + image_name + "_" + str(idx).zfill(3) + ".png"
            )
            ...
            f.write(
                "../" + mask_img_filename + " " + str(class_) + " " + str(score) + "\n"
            )
...
def visualize_detections(
    ...
    for idx, (bbox, score, class_, mask) in enumerate(
        ...
        if class_ == 1 or class_ == 2:  # Don't show boxes or ids for ignore regions
            ...
            ax.annotate(
                category_name,
                (bbox[0] + 0.5 * bbox[2], bbox[1] + 0.5 * bbox[3]),
                color=color,
                weight="bold",
                fontsize=7,
                ha="center",
