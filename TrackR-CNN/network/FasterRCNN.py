...
from network.FasterRCNN_utils import (
    decode_bbox_target,
    encode_bbox_target,
...
def fastrcnn_head(
    ...
    with tf.variable_scope("fastrcnn"):
        ...
        box_regression = FullyConnected(
            ...
            tower_setup,
            activation="linear",
            W_initializer=tf.random_normal_initializer(stddev=0.001),
        ).outputs[0]
        box_regression = tf.reshape(box_regression, (-1, num_hidden_box, 4))

        if reid_dim > 0:
            if reid_per_class:
                reid_features = FullyConnected(
                    "reid",
                    [feature],
                    (num_classes - 1) * reid_dim,
                    tower_setup,
                    activation="linear",
                ).outputs[0]
                reid_features = tf.reshape(
                    reid_features, (-1, (num_classes - 1), reid_dim)
                )
                best_class = tf.cast(
                    tf.argmax(classification, axis=-1) - 1, dtype=tf.int32
                )
                feature_indices = tf.stack(
                    [tf.range(tf.shape(best_class)[0], dtype=tf.int32), best_class],
                    axis=1,
                )
                reid_features = tf.gather_nd(reid_features, feature_indices)
            else:
                reid_features = FullyConnected(
                    "reid", [feature], reid_dim, tower_setup, activation="linear"
                ).outputs[0]
        else:
            reid_features = None
...
class FasterRCNN(Layer):
    ...
    """
        ...
        with tf.name_scope("anchors"):
            ...
            fm_anchors = tf.slice(
                all_anchors,
                [0, 0, 0, 0],
                tf.stack([shape2d[0], shape2d[1], -1, -1]),
                name="fm_anchors",
            )
        return fm_anchors
    ...
    def __init__(
        ...
        network_input_dict,
        tower_setup,
        fastrcnn_batch_per_img=256,
        result_score_thresh=0.05,
        reid_dimension=0,
        reid_per_class=False,
        reid_loss_per_class=False,
        reid_loss_worst_examples_percent=1.0,
        reid_loss_variant=0,
        reid_loss_factor=1.0,
        reid_measure="cosine",
        reid_adjacent_frames=False,
        class_agnostic_box_and_mask_heads=False,
        reid_loss_margin=0.2,
        ...
        with tf.variable_scope(name):
            rpn_label_logits, rpn_box_logits = rpn_head(
                inputs, 1024, NUM_ANCHOR, self.tower_setup
            )
        ...
        for batch_idx in range(batch_size):
            with tf.control_dependencies(control_inputs=final_boxes_list):
                with tf.variable_scope(name, reuse=True if batch_idx > 0 else None):
                    final_boxes, final_labels, final_masks, final_probs, reid_features, target_ids, final_reid_features, fastrcnn_box_loss, fastrcnn_label_loss, mrcnn_loss, rpn_box_loss, rpn_label_loss = self._create_heads(
                        batch_idx,
                        fm_anchors,
                        fm_shape,
                        rpn_box_logits,
                    ...
                    reid_features_and_target_ids_per_time.append(
                        (reid_features, target_ids)
                    )
                    if self.is_training:
                        losses.extend(
                            [
                        ...
                        self.add_scalar_summary(
                            fastrcnn_label_loss, "fastrcnn_label_loss"
                        )
                        self.add_scalar_summary(fastrcnn_box_loss, "fastrcnn_box_loss")
                    else:
                        self._add_test_measures(
                            batch_idx,
                            final_boxes,
                            final_probs,
                            final_labels,
                        ...
                        final_boxes_list.append(
                            final_boxes
                        )  # For training, didnt have any OOM errors
        ...
        for key in self.extractions_per_batch_idx[0].keys():
            ...
            self.extractions[key] = [
                extractions[key] for extractions in self.extractions_per_batch_idx
            ]

        if self.is_training and self._do_reid and self._reid_loss_factor > 0.0:
            reid_loss = create_reid_loss(
                reid_features_and_target_ids_per_time,
                self._reid_loss_per_class,
                self._reid_loss_worst_examples_percent,
                self._reid_loss_variant,
                self._reid_loss_factor,
                self._reid_measure,
                self._reid_adjacent_frames,
        ...
        if self.is_training:
            vars_to_regularize = tf.trainable_variables(
                "frcnn/(?:rpn|group3|fastrcnn|maskrcnn)/.*W"
            )
    ...
    def _create_heads(
        ...
        decoded_boxes = decode_bbox_target(
            self.bbox_decode_clip, rpn_box_logits, fm_anchors
        )  # fHxfWxNAx4, floatbox
        ...
        if self.provide_boxes_as_input:
            input_boxes = tf.squeeze(
                self.network_input_dict[DataKeys.BBOXES_TO_REFINE_x0y0x1y1], axis=0
            )
            image_shape2d_before_resize = self.network_input_dict[
                DataKeys.RAW_IMAGE_SIZES
            ]
            assert (
                image_shape2d_before_resize.shape[0] == 1
            ), "we assume batch size is 1 for now"
            image_shape2d_before_resize = tf.squeeze(
                image_shape2d_before_resize, axis=0
            )
            image_shape2d = self._image_shape2d
            old_height, old_width = (
                image_shape2d_before_resize[0],
                image_shape2d_before_resize[1],
            )
        ...
        if self.is_training:
            ...
            target_ids = self.network_input_dict[
                DataKeys.IDS
            ]  # If 0, ignore; but 1 for IS_CROWD==True
            target_ids = target_ids[batch_idx]
            valid_gts_mask = tf.logical_and(
                tf.greater(target_ids, 0), tf.equal(is_crowd, 0)
            )
            ...
            featuremap_boxes = self.network_input_dict[
                DataKeys.FEATUREMAP_BOXES
            ...
            rcnn_sampled_boxes, rcnn_labels, fg_inds_wrt_gt, rcnn_target_ids = sample_fast_rcnn_targets(
                ...
                gt_boxes,
                gt_labels,
                self.fastrcnn_batch_per_img,
                target_ids,
            )
            boxes_on_featuremap = rcnn_sampled_boxes * (1.0 / ANCHOR_STRIDE)
            # tf.add_to_collection('checkpoints', boxes_on_featuremap)
        else:
            gt_boxes, gt_labels, gt_masks, target_ids, featuremap_labels, featuremap_boxes = (
                [None] * 6
            )
            rcnn_sampled_boxes, rcnn_labels, fg_inds_wrt_gt, rcnn_target_ids = (
                None,
                None,
                None,
                None,
        ...
        if self.is_training:
            ...
            final_boxes, final_labels, final_masks, final_probs, final_reid_features = (
                None,
                None,
                None,
                None,
    ...
    def _create_fastrcnn_output(self, boxes_on_featuremap, rpn_input):
        ...
        def ff_true():
            ...
            fastrcnn_label_logits_, fastrcnn_box_logits_, fastrcnn_reid_features_ = fastrcnn_head(
                feature_fastrcnn_,
                self._num_classes,
                self._reid_dimension,
                self.tower_setup,
                self._reid_per_class,
                class_agnostic_box=self.class_agnostic_box_and_mask_heads,
            )
            if self._do_reid:
                return (
                    feature_fastrcnn_,
                    fastrcnn_label_logits_,
                    fastrcnn_box_logits_,
                    fastrcnn_reid_features_,
                )
            else:
    ...
    def _create_losses(
        ...
        encoded_boxes = encode_bbox_target(
            matched_gt_boxes, fg_sampled_boxes
        ) * tf.constant(FASTRCNN_BBOX_REG_WEIGHTS)
        ...
        if self._add_maskrcnn:
            ...
            mask_logits = maskrcnn_head(
                ...
                self._num_classes,
                self.tower_setup,
                class_agnostic_conv=self.class_agnostic_box_and_mask_heads,
            )  # #fg x #cat x 14x14
            ...
            target_masks_for_fg = tf.squeeze(
                target_masks_for_fg, 3, "sampled_fg_mask_targets"
            )
            mrcnn_loss = maskrcnn_loss(
                mask_logits,
                fg_labels,
                target_masks_for_fg,
    ...
    def _create_final_outputs(
        ...
        label_probs = tf.nn.softmax(
            fastrcnn_label_logits, name="fastrcnn_all_probs"
        )  # #proposal x #Class
        if self.class_agnostic_box_and_mask_heads:
            anchors = tf.tile(
                tf.expand_dims(proposal_boxes, 1), [1, 1, 1]
            )  # #proposal x #Cat x 4
        else:
            anchors = tf.tile(
                tf.expand_dims(proposal_boxes, 1), [1, self._num_classes - 1, 1]
            )  # #proposal x #Cat x 4
        decoded_boxes = decode_bbox_target(
            self.bbox_decode_clip,
            fastrcnn_box_logits / tf.constant(FASTRCNN_BBOX_REG_WEIGHTS),
            anchors,
        )
        decoded_boxes = clip_boxes(
            decoded_boxes, self._image_shape2d, name="fastrcnn_all_boxes"
        )
        if self.provide_boxes_as_input:
            n_proposals = tf.shape(proposal_boxes)[0]
            pred_indices = tf.stack(
                [tf.range(n_proposals), tf.zeros((n_proposals,), dtype=tf.int32)],
                axis=1,
            )
        ...
        if self._add_maskrcnn:
            ...
            def f1():
                roi_resized = roi_align(
                    rpn_input, final_boxes * (1.0 / ANCHOR_STRIDE), 14
                )
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    feature_maskrcnn = add_resnet_conv5(roi_resized, self.tower_setup)[
                        0
                    ]
                mask_logits = maskrcnn_head(
                    feature_maskrcnn,
            ...
            final_masks = tf.cond(
                tf.size(final_probs) > 0, f1, lambda: tf.zeros([0, 14, 14])
            )
    ...
    def _add_test_measures(
        ...
        if not self.is_training:
            orig_img_shape = self.network_input_dict[DataKeys.RAW_IMAGE_SIZES][
                batch_idx, ...
            ]
            orig_img_shape_f = tf.cast(orig_img_shape, tf.float32)
            image_shape2d_f = tf.cast(self._image_shape2d, tf.float32)
            scale = (
                image_shape2d_f[0] / orig_img_shape_f[0]
                + image_shape2d_f[1] / orig_img_shape_f[1]
            ) / 2
            boxes = final_boxes / scale
            clipped_boxes = clip_boxes(boxes, orig_img_shape_f)
            self.extractions_per_batch_idx[batch_idx][
                Extractions.DET_BOXES
            ] = clipped_boxes
            self.extractions_per_batch_idx[batch_idx][
                Extractions.DET_PROBS
            ] = final_probs
            self.extractions_per_batch_idx[batch_idx][
                Extractions.DET_LABELS
            ] = final_labels
            self.extractions_per_batch_idx[batch_idx][
                Extractions.REID_FEATURES
            ] = final_reid_features
            if self._add_maskrcnn:
                final_masks = tf.py_func(
                    self.fill_full_mask,
                    [clipped_boxes, final_masks, orig_img_shape],
                    tf.uint8,
                    name="fill_full_mask",
                )
                self.extractions_per_batch_idx[batch_idx][
                    Extractions.DET_MASKS
                ] = final_masks
            if DataKeys.IMAGE_ID in self.network_input_dict:
                self.extractions_per_batch_idx[batch_idx][
                    Extractions.IMAGE_ID
                ] = self.network_input_dict[DataKeys.IMAGE_ID]


def create_reid_loss(
    reid_features_and_target_ids_per_time,
    reid_loss_per_class,
    reid_loss_worst_examples_percent,
    loss_variant,
    loss_factor,
    distance_measure,
    adjacent_frames,
    ...
    assert (
        reid_loss_worst_examples_percent == 1.0
    ), "Hard mining currently not implemented"

    if distance_measure == "cosine" or distance_measure == "normalized_euclidean":
        reid_features_and_target_ids_per_time = [
            (tf.nn.l2_normalize(reid, axis=1), ids)
    ...
    def compute_measure(a, b):
        if distance_measure == "cosine":
            ...
            )  # cosine similarity would be without 1-
        elif (
            distance_measure == "euclidean"
            or distance_measure == "normalized_euclidean"
        ):
    ...
    id_fun = partial(
        _create_reid_loss_for_id,
        reid_loss_per_class=reid_loss_per_class,
        reid_loss_worst_examples_percent=reid_loss_worst_examples_percent,
    ...
    if adjacent_frames:
        ...
        for (reid_features_t, target_ids_t), (reid_features_tp1, target_ids_tp1) in zip(
            reid_features_and_target_ids_per_time,
            reid_features_and_target_ids_per_time[1:],
        ):
            ...
            reid_loss_t, normalization_t = tf.cond(
                tf.logical_and(
                    tf.size(reid_features_t) > 0, tf.size(reid_features_tp1) > 0
                ),
                f,
        ...
        all_target_ids = tf.concat(
            [ids for (_, ids) in reid_features_and_target_ids_per_time], axis=0
        )
        all_reid_features = tf.concat(
            [reid for (reid, _) in reid_features_and_target_ids_per_time], axis=0
        )
        ...
        reid_loss_per_id_fn = partial(
            id_fun,
            computed_measure=computed_measure,
            target_ids_axis_0=all_target_ids,
            target_ids_axis_1=all_target_ids,
        )
        unique_target_ids, _ = tf.unique(all_target_ids)
        reid_losses_per_id, normalization_per_id = tf.map_fn(
            reid_loss_per_id_fn, unique_target_ids, dtype=(tf.float32, tf.int32)
        )
        reid_loss = tf.reduce_sum(reid_losses_per_id, axis=0)
        normalization = tf.reduce_sum(normalization_per_id, axis=0)
        if loss_variant == 1:
            normalization = tf.size(
                all_target_ids
...
def _create_reid_loss_for_id(
    ...
    if reid_loss_per_class:
        ...
        class_mask_axis_1 = tf.equal(
            tf.floordiv(target_ids_axis_1, 1000), tf.floordiv(id_, 1000)
        )
    ...
    def triplet():
        if loss_variant == 2 or loss_variant == 3:  # batch all
            ...
            all_combinations = tf.expand_dims(same, axis=2) - tf.expand_dims(
                different, axis=1
            )
    ...
    if loss_variant == 4:
        loss, normalization = tf.cond(
            tf.logical_and(tf.size(same) > 0, tf.size(different) > 0),
            contrastive,
            lambda: (tf.constant(0.0), tf.constant(1, dtype=tf.int32)),
        )
    else:
