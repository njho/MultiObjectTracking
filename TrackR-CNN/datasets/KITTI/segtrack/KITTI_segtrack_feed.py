...
class KittiSegtrackLikeFeedDataset(FeedDataset):
    def __init__(
        ...
        super().__init__(
            config, subset, data_keys_to_use=DATA_KEYS_TO_USE, num_classes=NUM_CLASSES
        )
        ...
        if self.config.bool("offset_matching", False):
            ...
            print(
                ...
                "context frames so actual batch size will decrease",
            )
    ...
    def get_feed_dict_for_next_step(self):
        ...
        for idx in range(self._batch_size):
            ...
            if self._preload_images:
                feed_dict[self._placeholders[DataKeys.IMAGES][idx]] = self._imgs[
                    time_idx
                ]
            else:
                feed_dict[self._placeholders[DataKeys.IMAGES][idx]] = _load_img(
                    self._filenames[time_idx]
                )
            feed_dict[
                self._placeholders[DataKeys.IMAGE_FILENAMES][idx]
            ] = self._filenames[time_idx]
...
class KittiSegtrackFeedDataset(KittiSegtrackLikeFeedDataset):
    ...
    def get_filenames_for_video_idx(self, idx):
        print("get_filenames_for_video")
        print(self.data_dir)
        print(
            sorted(
                glob.glob(self.data_dir + "/images/" + self._video_tags[idx] + "/*.png")
            )
        )
        return sorted(
            glob.glob(self.data_dir + "/images/" + self._video_tags[idx] + "/*.png")
